...
export default function Home() {
	...
	const handleFormSubmit = async (event) => {
		...
		try {
			...
			await axios
				.put(`http://localhost:7166/api/Batteries/${id}`, requestData)
				.then((response) => {
	...
	useEffect(() => {
		...
		axios
			...
			.then((response) => {
				...
				const asset = response.data.find((asset) => asset.batteryId === id);
				const batterySerial = asset.stampedSerial;
				console.log('Battery serial: ' + batterySerial);
				if (batterySerial) {
					setFormState((prevState) => ({
						...prevState,
